...
model_path = "checkpoints/refhcm.pt"
input_args = ["", "--task=multitask", "--beam=5", f"--path={model_path}", "--bpe-dir=utils/BPE", "--patch-image-size=512"]
...
def build_prefix(prefix):
    prefix = encode_text(' {}'.format(prefix.lower().strip()), append_bos=True, append_eos=False).unsqueeze(0)
    prefix = utils.move_to_cuda(prefix) if use_cuda else prefix
    prefix = utils.apply_to_sample(apply_half, prefix) if use_fp16 else prefix
    return prefix
...
if __name__ == '__main__':
    ...
    if flags.par:
        prompt += ' Provide the parsing map'

    if flags.kpt:
        prompt += ' and the keypoints. '

    print(f"prompt:{prompt}")

    with torch.no_grad():
        hypos = task.inference_step(generator, models, build_sample(image, prompt))
        ...
        prefix = "" # used for induction

        # 1. model always perform REC
        coord_list = bin2coord(bins, w_resize_ratio, h_resize_ratio)
        print(f'region: {coord_list[:4]}')

        # 2. perform RKpt if flags.kpt is True
        if flags.kpt:
            while len(bins.strip().split()) <= 4:
                print('no keypoints, need induction!')
                prefix += " ".join(bins.strip().split()[:4]) # add region info
                prefix += ' nose ' # add keypoint indicator
                prefix = build_prefix(prefix)
                print(f"prefix:{prefix}")
                hypos = task.inference_step(generator, models, build_sample(image, prompt), prefix_tokens = prefix)
                tokens, bins, imgs, result = decode_fn(hypos[0][0]["tokens"], task.tgt_dict, task.bpe, generator)
                print(f"result:{result}")
            kpts = process_kpt(tokens, bins, w_resize_ratio, h_resize_ratio)

        prefix = result # update prefix to the last output result
        
        # 3. perform RPar if flags.par is True
        if flags.par:
            while len(imgs.strip().split()) == 0: # parsing need 6*8 = 48 latent codes
                print('no parsing maps, need induction!')
                print(prompt)
                prefix += " ".join(bins.strip().split()[:4]) # add region info
                prefix += ' <code_0>' # add parsing indicator
                prefix = build_prefix(prefix)
                print(f"prefix:{prefix}")
                hypos = task.inference_step(generator, models, build_sample(image, prompt), prefix_tokens = prefix)
                tokens, bins, imgs, result = decode_fn(hypos[0][0]["tokens"], task.tgt_dict, task.bpe, generator)

...
class RParDataset(OFADataset):
    ...
    def __getitem__(self, index):
        ...
        if text != 'None': # reference data
            ...
            # no reference, use bbox as condition
            src_list = []
            for item in self.prompt.split():
                if item != "{}":
                    src_list.append(self.bpe.encode(' {}'.format(item)))
                else:
                    src_list.extend([quant_x0, quant_y0, quant_x1, quant_y1])
            src_item = " ".join(src_list)
            src_item = self.encode_text(src_item, use_bpe=False)
            tgt_item = self.encode_text(region_coord + " " + parsing_code, use_bpe=False)

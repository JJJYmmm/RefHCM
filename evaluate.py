...
from utils.eval_utils import eval_step, merge_results, compute_miou
...
def main(cfg: DictConfig, **kwargs):
    ...
    if task.cfg._name == 'rpar' or task.cfg._name == 'rpar_full_mask':
        confusion_matrix = np.zeros((task.num_labels, task.num_labels))
    ...
    if task.cfg._name == 'rpar' or task.cfg._name == 'rpar_full_mask':
        confusion_matrix = torch.from_numpy(confusion_matrix).to(dist.get_rank())
        ...
        if dist.get_rank() == 0:
            ...
            macc, miou, pacc = compute_miou(task, confusion_matrix)
            logger.info("Final macc:{} miou:{} pacc:{}".format(macc, miou, pacc))
...
def cli_main():
    ...
    parser.add_argument("--arch", default='refhcm')

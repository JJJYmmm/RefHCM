...
class AdjustLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, update_num, reduce=True):
        ori_lprobs, ori_target, constraint_masks = self.get_lprobs_and_target(model, net_output, sample)
        if "full_mask_position" in sample["net_input"]: # calculate loss of non-auto-regressive tokens
            beg, end = sample["net_input"]["full_mask_position"]
            non_ar_lprobs = ori_lprobs[:, beg : end + 1]
            non_ar_target = ori_target[:, beg : end + 1]
            loss_2, _, ntokens_2 = label_smoothed_nll_loss(
                non_ar_lprobs,
                non_ar_target,
                self.eps,
                update_num,
            ...
            ar_lprobs = torch.cat([ori_lprobs[:,:beg], ori_lprobs[:,end+1:]], dim=1).view(-1, ori_lprobs.size(-1))
            ar_target = torch.cat([ori_target[:,:beg], ori_target[:,end+1:]], dim=1).view(-1)
        else: # only auto-regressive tokens
            ar_lprobs = ori_lprobs
            ar_target = ori_target
        if constraint_masks is not None:
            constraint_masks = constraint_masks[ar_target != self.padding_idx]
        lprobs = ar_lprobs[ar_target != self.padding_idx]
        target = ar_target[ar_target != self.padding_idx]
        ...
        if "full_mask_position" in sample["net_input"]: # calculate loss for non-auto-regressive tokens
            loss += loss_2
            ntokens += ntokens_2
        return loss, nll_loss, ntokens

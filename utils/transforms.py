...
def resize(image, target, size, max_size=None):
    ...
    if "keypoints" in target:
        ...
        scaled_kpt = keypoints * torch.as_tensor([item for _ in range(keypoints.size(1) // 2) for item in [ratio_width, ratio_height]])
        target["keypoints"] = scaled_kpt
...
class ResizeWithBoxes(object):
    ...
    def __call__(self, image, target):
        ...
        width_ratio = self.size / orig_width
        height_ratio = self.size / orig_height
        ratio = min(width_ratio, height_ratio)

        assert "boxes" in target
        new_boxes = target["boxes"].clone()
        new_boxes[:, 0] *= ratio  # x1
        new_boxes[:, 1] *= ratio  # y1
        new_boxes[:, 2] *= ratio  # x2
        new_boxes[:, 3] *= ratio  # y2

        new_width = int(ratio * orig_width)
        new_height = int(ratio * orig_height)
        transform = T.Resize((new_height, new_width))
        ...
        new_boxes[:, 0] += left_pad  # x1
        new_boxes[:, 1] += top_pad  # y1
        new_boxes[:, 2] += left_pad  # x2
        new_boxes[:, 3] += top_pad  # y2
    
        target["boxes"] = new_boxes

        # 对图像进行padding
        padding = (left_pad, top_pad, right_pad, bottom_pad)
        new_image = T.Pad(padding, fill=0, padding_mode='constant')(new_image)

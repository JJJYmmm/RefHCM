...
def build_vocab(imgs, params):
    ...
    for img in imgs:
        caption = pre_caption(img[1]).strip().split()
        caption = [w if counts.get(w,0) > count_thr else 'UNK' for w in caption]
        img.append(caption)
...
def encode_captions(imgs, params, wtoi):
    ...
    for i,img in enumerate(imgs):
        ...
        s = img[-1] # get final caption list
...
def main(params):
    ...
    # create the vocab
    vocab, imgs = build_vocab(imgs, params)
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_tsv', required=True, help='input tsv file to process into hdf5')
    parser.add_argument('--output_json', default='data.json', help='output json file')
    parser.add_argument('--output_h5', default='data', help='output h5 file')
    parser.add_argument('--images_root', default='', help='root location in which images are stored, to be prepended to file_path in input json')

    # options
    parser.add_argument('--max_length', default=80, type=int, help='max length of a caption, in number of words. captions longer than this get clipped.')
